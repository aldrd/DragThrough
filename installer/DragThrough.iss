...
#define MyAppPublisher "Alexey Redozubov, Valery Pogorelov"
#define MyAppURL "https://buymeacoffee.com/redozubov"
#define MyAppCopyright "Copyright (c) 2026 Alexey Redozubov, Valery Pogorelov"
...
; Shows the CC BY-ND 4.0 terms as a wizard page the user must accept. The repo-root LICENSE is
; the canonical Creative Commons text, byte for byte; Inno renders it as plain text (it only
; switches to RTF when the file starts with an RTF header), so the lack of an extension is fine.
LicenseFile=..\LICENSE
...
DisableDirPage=yes
OutputDir=Output
OutputBaseFilename=DragThrough-Setup
