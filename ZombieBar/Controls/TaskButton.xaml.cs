...
namespace ZombieBar.Controls
{
    ...
    public partial class TaskButton : UserControl
    {
        ...
        private void TaskButton_OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            StartExplorerPathRetry();

            _isLoaded = true;
        }

        // A freshly opened File Explorer window may not be registered with the shell yet when its
        // button is first bound, so ExplorerTitleConverter can't resolve the path on the first
        // pass and falls back to the title. Re-evaluate the title bindings a few times until the
        // path resolves (or give up after a few seconds for virtual locations like "This PC").
        private DispatcherTimer explorerPathTimer;
        private int explorerPathRetries;

        private void StartExplorerPathRetry()
        {
            ...
            explorerPathRetries = 0;
            explorerPathTimer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(400) };
            explorerPathTimer.Tick += ExplorerPathTimer_Tick;
            explorerPathTimer.Start();
        }

        private void StopExplorerPathRetry()
        {
            if (explorerPathTimer != null)
            {
                explorerPathTimer.Stop();
                explorerPathTimer.Tick -= ExplorerPathTimer_Tick;
                explorerPathTimer = null;
            }
        }

        private void ExplorerPathTimer_Tick(object sender, EventArgs e)
        {
            explorerPathRetries++;

            bool resolved = Window != null && !string.IsNullOrEmpty(ExplorerPathHelper.GetPath(Window.Handle));
            if (resolved)
            {
                RefreshTitleBindings();
            }

            if (resolved || explorerPathRetries >= 8)
            {
                StopExplorerPathRetry();
            }
        }

        private void RefreshTitleBindings()
        {
            // Only the visible labels show the resolved path; the tooltip binds to Title directly.
            BindingOperations.GetMultiBindingExpression(LeftLabel, ContentControl.ContentProperty)?.UpdateTarget();
            BindingOperations.GetMultiBindingExpression(RightLabel, ContentControl.ContentProperty)?.UpdateTarget();
        }
