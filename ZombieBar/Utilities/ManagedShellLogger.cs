...
namespace ZombieBar.Utilities
{
    ...
    {
        private string _logPath = Path.Combine(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ZombieBar"), "Logs");
        ...
        private TimeSpan _logRetention = new TimeSpan(7, 0, 0, 0); // 7 days (days, hours, minutes, seconds)
        ...
        {
            // Ensure the log directory exists before deleting old logs or opening a new one (first run).
