    ...
    public class TasksOrderManager : IDisposable
    {
        ...
        private void EnsureOrder(ApplicationWindow window)
        {
            ...
            if (!_orderByTitle.TryGetValue(title, out string order) || string.IsNullOrEmpty(order)
                || IsOrderInUseByLiveWindow(order))
            {
                // New title, or the remembered order is already taken by another live window. The latter
                // happens with a second concurrent window of the same app whose add-time title matches an
                // earlier one (e.g. two editor windows that both opened as "VSCodium"): reusing the same
                // order would leave both buttons with an identical sort key, and then nothing can be
                // dropped between them (a key equal to both neighbours can't sort strictly between them).
                // Give this window its own order at the end so every live window is independently
                // orderable. The remembered slot for the title is kept for the first window only.
                order = OrderHelper.GetOrder(MaxOrder(), "");
                _orderByTitle.TryAdd(title, order);
            }
        ...
        // Whether any currently-shown window already carries this order identifier.
        private bool IsOrderInUseByLiveWindow(string order)
        {
            foreach (ApplicationWindow window in _keyByWindow.Keys)
            {
                if (string.Equals(window.Order, order, StringComparison.Ordinal))
                    return true;
            }
            return false;
        }
        ...
        private string MaxOrder()
        {
            ...
            // Also consider live windows: a second same-title window's distinct order is intentionally
            // not stored in _orderByTitle, so it must still be counted here to keep new orders unique.
            foreach (ApplicationWindow window in _keyByWindow.Keys)
            {
                if (!string.IsNullOrEmpty(window.Order) && string.CompareOrdinal(window.Order, max) > 0)
                    max = window.Order;
            }
            return max;
        }
