...
namespace ManagedShell.AppBar
{
    public class AppBarWindow : Window, INotifyPropertyChanged
    {
        ...
        public AppBarWindow(AppBarManager appBarManager, ExplorerHelper explorerHelper, FullScreenHelper fullScreenHelper, ScreenInfo screen, AppBarEdge edge, double size)
        {
        ...
        private void FullScreenApps_CollectionChanged(object sender, System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            ...
            {
                if (app.screen.DeviceName == ScreenInfo.DeviceName || app.screen.IsVirtualScreen)
                {
        ...
        protected virtual IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            else if (msg == (int)NativeMethods.WM.DPICHANGED)
            {
                ...
                ScreenHelper.InvalidatePrimaryScreenScale();
        ...
        public void SetPosition()
        {
            ...
            else if (EnableAppBar)
            {
                if (Orientation == Orientation.Vertical)
                {
                    _appBarManager.AppBarWindowSetPosition(this, DesiredWidth * DpiScale, ActualHeight * DpiScale, AppBarEdge);
                }
                else
                {
                    _appBarManager.AppBarWindowSetPosition(this, ActualWidth * DpiScale, DesiredHeight * DpiScale, AppBarEdge);
                }
            }
        }
        ...
        private void ProcessScreenChange(ScreenSetupReason reason)
        {
            ...
            if (((ScreenInfo.Primary && ProcessScreenChanges) || reason == ScreenSetupReason.DpiChange) && !AllowClose)
            {
        ...
        private void setFullScreenMode(bool entering)
        {
            if (entering)
            {
                ShellLogger.Debug($"AppBarWindow: {Name} on {ScreenInfo.DeviceName} conceding to full-screen app");
            ...
            {
                ShellLogger.Debug($"AppBarWindow: {Name} on {ScreenInfo.DeviceName} returning to normal state");
        ...
        {
            if (!EnableAppBar || _appBarManager.AppBarWindowsCollection.Contains(this))
            {
                return;
            }
        ...
        {
            if (!_appBarManager.AppBarWindowsCollection.Contains(this))
            {
                return;
            }
        ...
        protected virtual void SetScreenProperties(ScreenSetupReason reason)
        {
            ...
            ScreenInfo = ScreenInfo.FromPrimaryScreen();
            SetPosition();
        }

        /// <summary>
        ///     Used only when executed as shell 
        /// </summary>
        public virtual void SetPositionSimple()
        {
            double screenEdgeOffset = _appBarManager.GetScreenEdgeOffset(AppBarEdge, ScreenInfo);

            var screenWorkArea = new Rect(ScreenInfo.Bounds.Left, ScreenInfo.Bounds.Top, ScreenInfo.Bounds.Width, ScreenInfo.Bounds.Height);
            switch (AppBarEdge)
            {
                case AppBarEdge.Left:
                    Width = DesiredWidth;
                    Height = screenWorkArea.Height / DpiScale;                    
                    Top = screenWorkArea.Top / DpiScale;
                    Left = screenWorkArea.Left / DpiScale + screenEdgeOffset;
                    break;
                case AppBarEdge.Top:                    
                    Width = screenWorkArea.Width / DpiScale;
                    Height = DesiredHeight;
                    Left = screenWorkArea.Left / DpiScale;
                    Top = (screenWorkArea.Top / DpiScale) + screenEdgeOffset;
                    break;
                case AppBarEdge.Right:
                    Width = DesiredWidth;
                    Height = screenWorkArea.Height / DpiScale;                    
                    Top = screenWorkArea.Top / DpiScale;
                    Left = screenWorkArea.Right / DpiScale - Width - screenEdgeOffset;
                    break;
                case AppBarEdge.Bottom:                    
                    Width = screenWorkArea.Width / DpiScale;
                    Height = DesiredHeight;
                    Top = screenWorkArea.Bottom / DpiScale - Height - screenEdgeOffset;
                    Left = screenWorkArea.Left / DpiScale;                    
                    break;
            }            
            ...
            {
                _appBarManager.SetWorkingArea(ScreenInfo);
            }
        }
        #endregion
